...
If `ADMIN_USERNAME` and `ADMIN_PASSWORD` are missing the server still starts and logs a warning. The open algorithm endpoints and `/api/examples` keep working, but the web UI cannot be used: every screen requires a session and there is no registration page, so with no administrator nobody can sign in and the page shows nothing but the login card. Both variables can be cleared once the account exists — the bootstrap never overwrites an existing account, so changing `ADMIN_PASSWORD` later does not reset a live password.
